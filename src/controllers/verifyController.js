...
async function processBySlug(req, res, next) {
  ...
  try {
    ...
    logger.info({
      ...
      stopReason: result.status === 'stopped' ? result.stopReason || result.reason : null,
      ...
      riskScore: result.riskAssessment?.overallScore ?? result.authenticity?.score ?? null,
      riskPassed: result.riskAssessment?.passed ?? result.authenticity?.passed ?? null,
